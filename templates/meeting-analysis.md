# 会议分析

## 基本信息

- Project: <project-id>
- Meeting: <meeting-id>
- Meeting Datetime: <meeting_datetime>
- Time Confidence: <confirmed|date_only|unknown>
- Source:
  - `metadata.yaml`
  - `transcript.md`
  - <artifacts if any>

## 会议目的

- <会议主要目的。>

## 背景和触发原因

- <背景事实。>
- <触发原因。>

## 已达成共识

- <共识 1>
- <共识 2>

## 明确决定

| ID | Decision | Domain | Status | Supersedes | Source |
| --- | --- | --- | --- | --- | --- |
| decision-YYYY-MM-DD-001 | <决定内容> | 方案/决策 | active |  | transcript.md |

## 未决事项

| ID | Question | Domain | Status | Owner | Source |
| --- | --- | --- | --- | --- | --- |
| question-YYYY-MM-DD-001 | <待确认问题> | 风险/依赖 | open | unknown | transcript.md |

## Todo

| ID | Todo | Domain | Status | Owner | Due | Source |
| --- | --- | --- | --- | --- | --- | --- |
| todo-YYYY-MM-DD-001 | <行动项> | 协作/责任 | open | unknown | unknown | transcript.md |

## 风险、依赖和阻塞

- <风险 / 依赖 / 阻塞。>

## 需要更新到项目知识库的内容

- `current-summary.md`: <需要更新的摘要。>
- `by-domain.md`: <需要更新的 domain。>
- `current-decisions.md`: <新增或变更的决定。>
- `current-open-questions.md`: <新增或关闭的未决事项。>
- `current-todos.md`: <新增或状态变更的 todo。>
- `timeline.md`: <时间线变化。>

## 来源证据

- <引用时间点、发言人、页码、附件名或片段摘要。>
